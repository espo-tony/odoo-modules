...
class stock_picking_discount(orm.Model):
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id, group=False, type='out_invoice', context=None):
        ...
        for invoice_id in invoices:
            ...
            invoice_lines = self.pool.get('account.invoice').browse(cr,uid,invoice_id).invoice_line
            for inv_line in invoice_lines:
                if inv_line.document_reference_id:
                    for mov_line in inv_line.document_reference_id.move_lines:
                        if mov_line.procurement_id and mov_line.procurement_id.sale_line_id and mov_line.procurement_id.sale_line_id.order_id:
                            discount_line = mov_line.procurement_id.sale_line_id.order_id.global_discount_lines
                            found = True
                            break
                    if found:
                        break
            ...
            for line in discount_line:
                discount_line_ids.append(self.pool.get('account.invoice.discount').create(cr,uid,{'invoice_id':invoice_id, 'name':line.name.id,'type':line.type, 'application':line.application, 'sequence':line.sequence,'value':line.value}))
            if discount_line_ids:
                self.pool.get('account.invoice').write(cr,uid,invoice_id,{'global_discount_lines':[(6,0,discount_line_ids)]})           
        '''    
        return invoices
