...
class account_invoice_with_discount(models.Model):
    ...
    amount_untaxed_free = fields.Float(string='"For Free" Amount', digits_compute=dp.get_precision('Account'), store=True, compute='_compute_amount')
    amount_tax_free = fields.Float(string='"For Free" Tax', digits_compute=dp.get_precision('Account'), store=True, compute='_compute_amount')
